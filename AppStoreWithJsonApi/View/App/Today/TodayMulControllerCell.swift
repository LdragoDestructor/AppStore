...
class TodayMulControllerCell:UICollectionViewCell{
    
    
    var result:Results?{
        didSet{
            appName.text = result?.name
            companyName.text = result?.artistName
            imageName.sd_setImage(with: URL(string: result?.artworkUrl100 ?? ""))
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        seperator.anchor(top: companyName.bottomAnchor, leading: companyName.leadingAnchor, bottom: nil, trailing: trailingAnchor,padding: .init(top: 20, left: 0, bottom: 0, right: 0))
    }
